...
class Products extends Action
{
    ...
    public function products($collection)
    {
        ...
        foreach ($collection as $_product):
            $html .= $iterator++ == 1 ? '<li class="item product product-item">' : '</li><li class="item product product-item">';
        ...
        $html .= '<a href="' . $_product->getProductUrl() . '" class="product photo product-item-photo" tabindex="-1">' . $productImage->toHtml();
        $html .= '</a>';
        ...
        $html .= '<a class="product-item-link" title="' . $_product->getName() . '"
             href="' . $_product->getProductUrl() . '">' . $this->helper->productAttribute($_product, $_product->getName(), 'name');
        $html .= '</a></strong>';
        if ($showCart):
            $html .= '<div class="actions-primary">';
        if ($_product->isSaleable()):
            $html .= '<div class="product details product-item-details products-textlink clearfix">';
        $html .= '<div class="price-review">';
        $html .= $this->slider->getProductPrice($_product);
        $html .= $showReview ? $this->slider->getReviewsSummaryHtml($_product, $templateType) : '';
        $html .= $this->slider->getProductDetailsHtml($_product);
        $html .= '</div></div>';
        $postParams = $this->slider->getAddToCartPostParams($_product);
        if ($_product->getTypeId() == Configurable::TYPE_CODE || $_product->getTypeId() == Type::TYPE_DOWNLOADABLE) {
            $html .= '<strong class=product-item-configurable>';
            $html .= '<a id="product-addtocart-button" class="action tocart primary" title="' . $_product->getName() . '" href="' . $_product->getProductUrl() . '">Add to Cart';
            $html .= '</a></strong>';
        } else {
            $html .= '<form data-role="tocart-form" data-product-sku="' . $_product->getSku() . '" action="' . $postParams['action'] . '" method="post">';
            $html .= '<input type="hidden" class="product-id" name="product" value="' . $postParams['data']['product'] . '" />';
            $html .= '<input type="hidden" name="' . Action::PARAM_NAME_URL_ENCODED . '" value="' . str_replace(",,", ",", $postParams['data'][Action::PARAM_NAME_URL_ENCODED]) . '" />';
            $html .= '<input name="form_key" type="hidden" value="' . $this->slider->getFormKey() . '" />';
            $html .= '<button id="product-addtocart-button" class="action tocart primary" type="button" title="' . $this->slider->escapeHtml(__('Add to Cart')) . '" key=' . $_product->getId() . ' class="action tocart primary button btn-cart pull-left-none">';
